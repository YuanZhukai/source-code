...
namespace pofPrototype
{
    ...
    public class simController
    {
        ...
        public simController(int n, int latency)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                peer_obj p = new peer_obj(i,latency);
                p.localDist = dist;
        ...
        public void report()
        {
            ...
            //Console.WriteLine(datar.avgIT + "," + datar.minIT + "," + datar.maxIT + "," + avgTX + "," + datar.minTX.TotalMilliseconds + "," + datar.maxTX.TotalMilliseconds + "," + totTX + "," + totCheek);
            Console.WriteLine("latency:" + latency);
            Console.WriteLine("#peers:" + n_peers);
            //Console.WriteLine(avgTX);
            double fraction = (transactions * 100000) / (transactions * avgTX);
            if (fraction > 1.0)
            {
                fraction = 1.0;
            }
            Console.WriteLine("verified:" + fraction);
            Console.WriteLine("----------------------------------");
        }
